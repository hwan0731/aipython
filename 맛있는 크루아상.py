...
def display_animal_image(animal):
    """선택된 동물에 따라 이미지를 표시합니다."""
    image_path = f"{animal}.jpg"  # 이미지 파일 이름 규칙 (dog.jpg, cat.jpg, rabbit.jpg)
    try:
        ...
        if hasattr(window, "image_label"):
            window.image_label.destroy()

        # 새로운 이미지 레이블 생성 및 표시
        window.image_label = tk.Label(window, image=photo)
        window.image_label.image = photo  # 이미지 객체 유지
        window.image_label.pack()
        error_label.config(text="")  # 오류 메시지 초기화

    except FileNotFoundError:
        error_label.config(text=f"파일을 찾을 수 없습니다: {image_path}")
    except Exception as e:
        error_label.config(text=f"오류 발생: {e}")

# 메인 윈도우 생성
window = tk.Tk()
window.title("귀여운 동물 친구들")

# 버튼 생성
dog_button = tk.Button(window, text="크루아상", command=lambda: display_animal_image("크루아상상"))
dog_button.pack(pady=5)

cat_button = tk.Button(window, text="고양이", command=lambda: display_animal_image("cat"))
cat_button.pack(pady=5)

rabbit_button = tk.Button(window, text="토끼", command=lambda: display_animal_image("rabbit"))
...
def display_croissant():
    """크루아상 이미지를 윈도우에 표시합니다."""
    image_path = "croissant.jpg"  # 크루아상 이미지 파일 이름
    try:
        image = Image.open(image_path)
        photo = ImageTk.PhotoImage(image)
...
# 버튼 생성
croissant_button = tk.Button(window, text="크루아상 보기", command=display_croissant)
croissant_button.pack(pady=10)
...
import tkinter as tk
from PIL import Image, ImageTk
import os  # 파일 경로 확인에 유용할 수 있습니다.

def display_croissant():
    """크루아상 이미지를 윈도우에 표시합니다."""
    image_path = "croissant.jpg"  # 이미지 파일 이름 및 경로를 정확히 확인하세요.
    print(f"이미지 파일 경로 확인: {os.path.abspath(image_path)}") # 현재 이미지 파일이 어디에 있는지 출력
